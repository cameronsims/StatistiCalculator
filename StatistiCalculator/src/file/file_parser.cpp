...
void statcalc::files::read_input(Dataset& p_dataset, size_t argc, const wchar_t** argv) {
    // Check if there is files
    if (argc > 1) {
        // Add files to vector
        Vector<std::wstring> files(argc - 1);
...
void statcalc::files::read_inputFiles(Vector<std::wstring>& p_files, size_t argc, const wchar_t** argv) {
    // Loop all files
...
void statcalc::files::read_files(statcalc::Dataset& p_dataset, const Vector<std::wstring>& p_files) {
    ...
    for (int i = 0; i < SIZE; i++) {
        // File name
        std::wstring fileName = p_files[i];
        statcalc::menu::print_fileLoading(fileName);
        ...
        try {
            ...
            statcalc::menu::print_fileNotFound(fileName);
...
void statcalc::files::read_file(statcalc::Dataset& p_dataset, const std::wstring& p_fileName, bool p_header) {
    // The input stream
    std::wifstream in(p_fileName);
...
void statcalc::files::read_head(Vector<std::wstring>& p_header, const std::wstring& p_line) {
    // Important variables
    ...
    for (size_t i = 0; i < SIZE; i++) {
        if (p_line[i] == statcalc::options::seperator) {
            ...
            const std::wstring str = p_line.substr(start, end - start);
            p_header.push(str);
        }
    }
...
void statcalc::files::read_line(statcalc::Dataset& p_dataset, const std::wstring& p_line, size_t p_row) {
    // Length of wstring
    const std::wstring& STR = p_line;
...
void statcalc::files::read_cell(statcalc::Dataset& p_dataset, const std::wstring& p_line, size_t p_column, size_t p_row, size_t p_start, size_t p_end) {
    // Check if the data we have is a number
    const std::wstring& STR = p_line;
    const std::wstring SUBSTR = STR.substr(p_start, p_end - p_start);
